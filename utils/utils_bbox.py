        ...
        for i in range(len(mbox_loc)):
            ...
            for c in range(1, self.num_classes):
                ...
                if len(c_confs[c_confs_m]) > 0:
                    ...
                    selected_indices = tf.image.non_max_suppression( boxes_to_process,confs_to_process, self._top_k, iou_threshold=self._nms_thresh)
                    ...
                    good_boxes = tf.gather(boxes_to_process, selected_indices)
                    confs      = tf.gather(confs_to_process, selected_indices)
                    confs      = np.reshape(confs,(len(confs),1))
                    labels     = (c - 1) * np.ones((len(selected_indices), 1))
                    print('good_boxes:',good_boxes.shape)
