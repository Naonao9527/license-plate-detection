...
class SSD(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.anchors                        = get_anchors(self.input_shape, self.anchors_size,self.aspect_ratios)
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        if 检测:    
            ...
            for i, c in list(enumerate(top_boxes)):
                ...
                bottom  = min(image.size[1], np.floor(bottom).astype('int32'))
                right   = min(image.size[0], np.floor(right).astype('int32'))
                result = self.reader.readtext(np.asarray(image)[top:bottom,left:right],detail=0)
                if len(result)!=0:
                    print("result:",result)
                    if '99' in result[-1]:
